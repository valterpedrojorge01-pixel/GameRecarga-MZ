...
function render(list){grid.innerHTML=list.map(g=>`<article class="game"><div class="game-cover"><img src="${g.image}" alt="${g.name}" loading="lazy"><span>${g.mark}</span></div><div class="game-body"><h3>${g.name}</h3><p>Pacotes disponíveis</p><div class="packs">${g.packs.map(p=>`<button class="pack" title="${g.name} — ${p[0]}" onclick="openStore()">${p[0]}<b>${money(p[1])}</b></button>`).join('')}</div><button class="buy" onclick="openStore()">Comprar agora</button></div></article>`).join('')}
